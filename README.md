...
**Публичная ссылка пока не работает — приложение не опубликовано.**

Выдумывать адрес здесь нечего: ссылка появится в этой строке тогда, когда
по ней действительно откроется приложение, и ни минутой раньше.

Публикация настроена и проверена, но упирается в одну настройку, которую
может включить только владелец репозитория:

> **Settings → Pages → Source: GitHub Actions**

После этого выкладка пойдёт сама (`.github/workflows/deploy-pages.yml`), и
адрес появится здесь. Почему это нельзя сделать из workflow — в
[`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) §2.

**Пока ссылки нет, приложение можно запустить у себя** — см.
[«Запустить локально»](#запустить-локально). Это единственное место в
README, где обычному пользователю нужна командная строка, и оно временное.
...
Публикация на GitHub Pages настроена
(`.github/workflows/deploy-pages.yml`) и ждёт единственного шага —
включения Pages в настройках репозитория.
