...
export const WORKFLOW_STEPS: readonly WorkflowStep[] = [
  ...
  },
  {
    ...
    index: 6,
    title: 'Наполнение',
    // Штанга здесь НЕ названа намеренно: в списке наполнения её нет
    // (`editor/fill-vocabulary.ts`), потому что движок её не строит.
    // Подпись шага — такое же обещание пользователю, как и сам список;
    // разойтись они не должны, и это закреплено проверкой.
    hint: 'Что стоит внутри ячейки: полки или ящики.',
    screen: 'editor',
  },
  {
    id: 'facades',
  ...
  /*
    Шаг 10 — в КОНСТРУКТОРЕ, и это исправление FR-10 (PROMPT 64).

    Он объявлял `screen: 'production'`, как и шаг 11, а экран
    производства имеет собственную навигацию по разделам, к шагу не
    привязанную. Поэтому оба шага открывали один и тот же экран в одном
    и том же разделе: замер на собранном приложении дал 1817 знаков
    текста у обоих, совпадающих посимвольно, и 19 общих органов
    управления при нуле своих.

    Одиннадцать шагов обещали одиннадцать разных задач, а последние две
    были одной. Шагу 10 возвращено собственное содержимое — состав
    изделия и его незакрытые места, — и вопрос конструктора:
    «достроено ли изделие». Вопрос производства остался шагу 11.

    Назван «Обзор», а не «Проверка»: слово «Проверка» было источником
    путаницы и в этом шаге, и в разделе производства, который PROMPT 63
    переименовал в «Готовность». Третьего места с этим словом не
    заводится.
  */
  {
    id: 'validation',
    index: 10,
    title: 'Обзор',
    hint: 'Что уже собрано в изделии и какие места остались открытыми.',
    screen: 'editor',
  },
  {
    id: 'production',
