...
export function Scene3D(props: Scene3DProps): React.JSX.Element {
  ...
  const onPointerDown = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      /*
        Захват указателя обязателен: без него движение перестаёт
        приходить, как только курсор ушёл за пределы холста, и вращение
        срывается.

        Но он может и НЕ УДАСТЬСЯ: `setPointerCapture` бросает, если к
        моменту вызова активного указателя с таким id уже нет — палец
        подняли между событием и обработчиком, жест перехватила система,
        событие пришло синтетическим. Раньше это исключение уходило
        наружу и поднимало полосу «Приложение столкнулось с
        непредвиденной ошибкой» поверх целого приложения из-за
        неудавшегося захвата в одном холсте (PROMPT 54 §16 — правка,
        которую потребовала проверка P0).

        Отказ здесь не беда: без захвата жест просто закончится, когда
        указатель уйдёт с холста. Это заметно хуже, чем с захватом, и
        несравнимо лучше, чем сообщение об ошибке на весь экран.
      */
      try {
        host.setPointerCapture(event.pointerId);
      } catch {
        // Захвата не будет — жест продолжается без него.
      }
