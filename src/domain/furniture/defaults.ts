...
/**
 * Шариковые направляющие полного выдвижения, боковой монтаж.
 *
 * `sideClearance` — ПОДТВЕРЖДЁННАЯ величина (PROMPT 65 §3, бывший
 * `T-DRW-02`, теперь `T-DRW-06`): боковая шариковая направляющая занимает
 * **12.7 мм** с каждой стороны короба. Это не «примерно столько», а
 * толщина самой направляющей: Hettich обозначает её прямо в наименовании
 * изделия — «KA 5632 ball bearing runner, side installation, dimensions
 * (H × W) 46 × 12.7 mm», «KA 4932 … 50 × 12.7 mm»; Accuride для серии
 * 3832 требует 0.5″ (12.7 мм) свободного места с каждой стороны.
 * Отраслевое правило из того же ряда источников: короб ровно на 1″
 * (25.4 мм) уже внутреннего проёма. Ссылки — в
 * `docs/FR04_DRAWER_MODEL_ANALYSIS.md` §3.
 *
 * До PROMPT 65 здесь стояло 13 — округление «на глаз», которое сделало
 * бы каждый короб на 0.6 мм уже нужного. Число это пока НИЧЕМ не
 * читается (короб не строится, `T-DRW-07`), поэтому исправление не
 * меняет ни одной производственной величины — но модель перестаёт
 * хранить неверное.
 *
 * `nominalLength` остаётся `ASSUMPTION(T-DRW-03)`: ряд длин 250…600 с
 * шагом 50 в продаже есть (Knape & Vogt 4500, King Slide 3330), но
 * правило выбора длины по глубине корпуса источником не подтверждено.
 */
