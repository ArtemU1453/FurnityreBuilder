...
> `dividers` — §5.2; он же выпускает секции как области), `fill` (полки и
> фасады ящиков) и `facades` (распашные двери, базовый случай — одна
> ячейка на фасад), слой валидации, сериализация с версионированием,
> `ProjectRepository`, сторы документа и сессии с историей на патчах
> (включая `SetRoot` для атомарной замены дерева и `SetSectionCount` для
> изменения числа секций с сохранением идентичности), контроллер жестов,
> пружинный движок, токены design system, оболочка приложения с
> технической панелью секций, сетки, полок, дверей и ящиков.
...
> ячеек, присадка, интерактивная (не debug) отрисовка схемы, экспорт,
...
4. fill           — наполнение ячеек: Content → Parts                      ПОЛКИ И ФАСАДЫ
                     через resolveContentGeometry (§5.10);                 ЯЩИКОВ РЕАЛИЗОВАНЫ
                     фасады ящиков — resolveDrawerFacadeGeometry           (PROMPT 6, 11);
                     (PROMPT 11); короб ящика и штанга дают статус         короб: этап 21,
...
**Наполнение (`fill`) — полки (PROMPT 6) и фасады ящиков (PROMPT 11)
реализованы, короб ящика и штанга нет.** Формулы полки — в
`docs/GEOMETRY_RULES.md` §14, фасада ящика — §19.3, фасада двери — §18.3,
в том же формате, что и раскладка секций. Черновик ниже («Ящик», «Фасад»)
