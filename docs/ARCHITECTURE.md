...
> **Состояние реализации: продукт завершён, версия 0.1.0.**
>
> Реализовано всё, что перечислено в `docs/RELEASE_NOTES.md`: доменная
> модель, движок геометрии из пяти этапов, валидация, расчёт фурнитуры,
> раскроя и присадки, спецификация, экспорт PDF и XLSX, трёхмерная
> сцена на WebGL 2, планировщик помещения, библиотека проектов,
> хранение в IndexedDB с версионированием схемы, работа без сети.
>
> Чего в продукте нет и почему — `docs/PRODUCT_CAPABILITY_TRUTH.md` и
> `docs/KNOWN_ISSUES.md`. Коротко: короб ящика и штанга не строятся
> (правило не подтверждено), фасады купе/складные/подъёмные не
> реализованы, фасад покрывает одну ячейку.
>
> **Этот заголовок раньше врал.** До PROMPT 49 здесь стояла сводка,
> замороженная на PROMPT 9: она сообщала, что не реализованы присадка,
> экспорт и планировщик, — всё это работает и покрыто тестами. Первое,
> что читал новый разработчик в главном архитектурном документе, было
> описание проекта двухлетней давности по меркам его развития. Заменено
> на состояние, которое сверяется с `RELEASE_NOTES.md`, а не с памятью.
...
Parts / Hardware       — деталировка, кромка, крепёж, присадка
  ↓
Export Engine          — PDF и XLSX (CSV нет: см. RELEASE_NOTES.md)
  ↓
...
src/
  ...
  app/                       приложение
    shell/                   оболочка: верхняя строка, навигация, строка состояния
    screens/                 экраны: помещение, производство, общая раскладка
    production/              разделы производства, отбор деталировки и
                             трассируемость (чистые: ни React, ни DOM —
                             см. docs/PRODUCTION_UI.md)
    workflow/                пошаговый сценарий редактора: порядок шагов
                             и лестница (steps.ts — чистый: ни React, ни
                             DOM). Не путать со слоем src/workflow/ выше:
                             тот считает готовность к производству, этот
                             только раскладывает работу по шагам — та же
                             пара, что docs/EDITOR_WORKFLOW.md и
...
`state`/`interaction`. Управление (кнопка деления отделения, переключатель
