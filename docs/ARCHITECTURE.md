...
> **Состояние реализации.** Этап «Полки и горизонтальные элементы корпуса»
> завершён (PROMPT 6). Полка стала настоящей деталью движка: этап `fill`
> строит её внутри уже посчитанных ячеек, не создавая при этом ни одной
> новой ячейки — Cell остаётся пространством, Shelf является Part
> (`docs/GEOMETRY_RULES.md` §14). До этого (PROMPT 4) заработала раскладка
> дерева секций (§5.7) и появился первый рендерер — технический debug-слой
> `src/render/` (§10.1), отрисовывающий именно то, что посчитал движок,
> без собственной геометрии; PROMPT 5 был ревизией правил корпуса без
> изменения формул (§13 в `GEOMETRY_RULES.md`).
...
> `dividers` — §5.2) и `fill` (полки), слой валидации, сериализация
> с версионированием, `ProjectRepository`, сторы документа и сессии
> с историей на патчах (включая команду `SetRoot` для атомарной замены
> дерева), контроллер жестов, пружинный движок, токены design system,
> оболочка приложения с интерактивной технической панелью сетки и полок.
> Не реализованы: ящики и штанга (`LeafFill`-виды без геометрии), фасады,
...
4. fill           — полки внутри листовых ячеек                            ПОЛКИ РЕАЛИЗОВАНЫ;
                     ящики и штанги — ещё нет                              ящики: этап 21,
                                                                           штанга: этап 23
...
**Наполнение (`fill`) — полки реализованы (PROMPT 6), ящики и штанга нет.**
Формулы полки переехали в `docs/GEOMETRY_RULES.md` §14 в том же формате,
что и раскладка секций; здесь остаётся только рабочий черновик формул
ящика и фасада — они описывают этапы плана 21–22, а не текущий код.
