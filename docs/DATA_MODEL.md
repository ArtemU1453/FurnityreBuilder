...
> На PROMPT 9 зафиксирована модель наполнения ячейки: `LeafFill` (§5.5)
> объявлен тем самым `Content`, у него появился явный контракт резолвера
> и статус `not-implemented` вместо тихого пропуска нереализованных видов.
> Новых типов не добавлено.
...
**`LeafFill` — это и есть модель наполнения (Content) из PROMPT 9.**
Отдельного типа `Content { id, cellId, type, config }` рядом не заводится:
наполнение ЛЕЖИТ внутри ячейки, поэтому связь структурная, а не
поддерживаемая ссылкой. Отсюда четыре свойства по построению, которые
иначе пришлось бы обеспечивать вручную: ссылка на ячейку не может
испортиться (её нет), наполнение не может осиротеть, не может переехать
в другую ячейку при пересчёте и не может разойтись с ней в размерах.

| Понятие PROMPT 9 | Эквивалент в модели |
| --- | --- |
| `Content` | `LeafFill` — размеченное объединение с дискриминантом `kind` |
| `ContentType` | `LeafFill['kind']`, он же `ContentKind` в `src/geometry/content.ts` |
| `ContentConfig` | поля конкретного варианта: `shelves`, `drawers`, `rod` |
| `Content.id` | id ячейки (`LeafNode.id`): наполнение у ячейки одно, второго id заводить нечему |
| `ContentPlacement` | `Shelf.placement` и аналоги внутри варианта — привязка к ячейке, а не к мировым координатам |
| `cellId` | не поле, а вложенность: наполнение достаётся только вместе со своей ячейкой |
| `DOOR` | НЕ `LeafFill`: фасад закрывает несколько ячеек, поэтому живёт в `Furniture.facades` (§7) |

Правило «одно наполнение на ячейку» и точка расширения для новых видов —
...
Материал — справочные данные, геометрия детали — производный результат
(PROMPT 13). Конвейер один: **Material Registry → Part → эффективная
толщина → геометрия**; второй системы материалов в проекте нет, и
`MaterialLibrary` ниже и есть тот самый «Material Registry» из
формулировки задания — он существует с PROMPT 1, поэтому PROMPT 13 не
заводил рядом второй реестр, а достроил уже существующий.

### 9.1. Material и MaterialRegistry

Идентичность материала — `MaterialId`, НЕ имя: два материала могут
называться одинаково («ЛДСП 16»), переименование материала не должно
рвать ссылки на него, а ссылки деталей (`Shelf.materialId` и аналоги)
хранят именно id.
...
export interface Material {
  ...
  kind: 'chipboard' | 'mdf' | 'plywood' | 'hardboard' | 'solid' | 'glass' | 'mirror' | 'other';
  thickness: Mm;                // источник геометрии, а не справочная пометка
...
export interface MaterialLibrary {
  ...
  /** Назначение материала по ролям деталей — уровень наследования «корпус». */
  assignment: Partial<Record<PartRole, MaterialId>>;
}
```

Поддержанные категории (`MaterialKind`): `chipboard` (ЛДСП),
`mdf`, `plywood`, `hardboard`, `solid` (массив), `glass`, `mirror`,
`other`. Список закрыт намеренно: категория влияет на правила проверки
(стекло и зеркало на несущей роли — предупреждение, `T-MAT-04`), поэтому
придумывать категории, которых референс не подтвердил, нельзя — для всего
остального есть `other`.

### 9.2. Наследование материала и переопределение на детали

Второго типа «`PartMaterialOverride`» не заведено: переопределением уже
служит поле `materialId?` самой детали — оно существовало с PROMPT 1 у
`Shelf`, `DividerSpec`, `FacadeLeaf`, `DrawerFacadeSpec`, `DrawerBoxSpec`.
Уровни наследования, от общего к частному:

| Уровень | Где живёт | Что задаёт |
|---|---|---|
| Проект | `settings.defaultMaterialId` | материал проекта по умолчанию |
| Роль (корпус/фасад/полка/задняя стенка) | `materials.assignment[role]` | материал всех деталей этой роли |
| Деталь | `Shelf.materialId`, `DividerSpec.materialId`, `FacadeLeaf.materialId`, `DrawerFacadeSpec.materialId`, `DrawerBoxSpec.materialId` | материал одной конкретной детали |

Одна деталь всегда однозначно знает свой эффективный материал: его
вычисляет единственная функция `resolveEffectiveMaterial()`
(`src/geometry/parts.ts`, правила — `docs/GEOMETRY_RULES.md` §21).
Корпус и фасады могут быть из разных материалов, полки — из третьего,
фасад ящика — из своего, а короб ящика (`DrawerBoxSpec.materialId`) —
отдельно от своего фасада: это разные поля, и фасадный материал короба
не подменяет.

### 9.3. Кромка (EdgeBand)

Кромка — параметр детали, а не отдельная физическая деталь: у неё нет ни
`Part`, ни собственной геометрии, только толщина по каждой стороне и
(опционально) свой материал.
...
### 9.4. Стороны кромки: четыре, а не шесть

`EdgeSpec` описывает ЧЕТЫРЕ стороны (`front`/`back`/`left`/`right`) в
ЛОКАЛЬНЫХ координатах детали, а не шесть (`TOP`/`BOTTOM`/`LEFT`/`RIGHT`/
`FRONT`/`BACK`) в координатах изделия. Причина не в экономии: плоская
пластовая деталь имеет ровно четыре торца — пласти (две большие
плоскости) кромкой не оклеиваются никогда. Соответствие локальных сторон
осям изделия задаёт `Part.orientation`, и оно уже разное для разных
ролей: у боковины (`vertical-yz`) локальный `left` идёт по вертикали, у
полки (`horizontal-xz`) — по горизонтали. Держать вместо этого шесть
сторон в мировых осях означало бы, что «кромка сверху» у боковины и у
полки — про разные торцы, и правило «видимый фронтальный торец — 2 мм»
пришлось бы переписывать под каждую роль отдельно. Полная таблица
соответствий — `docs/COORDINATE_SYSTEM.md` §5.

Какие стороны осмысленны, зависит от роли детали (у задней стенки
кромки нет вообще, у полки значимы `front` и боковые торцы) — за это
отвечают правила материала роли, а не отдельный тип `EdgeSide`.
...
Строка итоговой спецификации (`HardwareLine`) здесь БЫЛА и удалена на
PROMPT 19: её место заняла `HardwareBomLine` (`src/hardware/types.ts`,
PROMPT 16), которая знает не только количество и детали-источники, но и
единицу измерения, категорию и полные позиции с причиной появления. Две
строки спецификации рядом — это два ответа на один вопрос, и второй
неизбежно отстал бы от первого.
