...
/** Проект без формата листа: детали негде разложить. */
export function withoutSheets(project: Project): Project {
  return {
    ...project,
    materials: {
      ...project.materials,
      items: Object.fromEntries(
        Object.entries(project.materials.items).map(([id, material]) => {
          const { sheet: _sheet, ...rest } = material;
          return [id, rest];
        }),
      ),
    },
