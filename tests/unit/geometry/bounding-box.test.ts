...
describe('computeBoundingBox: интеграция с buildGeometry', () => {
  it('для цельного корпуса совпадает с номинальным габаритом', () => {
    ...
    expect(result.boundingBox).toEqual({
      ...
      minY: 0,
      maxY: 2000,
      // С PROMPT 14 задняя стенка — деталь (z ∈ [0, 3]), поэтому охват
      // начинается с нуля и совпадает с номинальной глубиной целиком.
      // До этого этапа стенка вычиталась из глубины корпуса, но детали не
      // давала, и minZ равнялся 3 — охват «терял» 3 мм изделия.
      minZ: 0,
      maxZ: 500,
      totalWidth: 1000,
      totalHeight: 2000,
