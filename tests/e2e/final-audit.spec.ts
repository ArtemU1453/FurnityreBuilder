...
test('кнопка деления называет и полки, которые поставит (§3)', async ({ page }) => {
  await step(page, 'Ячейки').click();
  const primary = page.getByRole('button', { name: /^Разделить/ });

  /*
    С PROMPT 55 главное действие шага делит ВЫБРАННОЕ отделение, а не
    заменяет изделие (FR-02). Проверяемое свойство прежнее и стало
    строже: подпись называет всё, что действие сделает, — и вдобавок то,
    НАД ЧЕМ оно это сделает.
  */
  await expect(primary).toHaveText('Разделить «Всё внутреннее пространство» на 1×1');
  ...
  await expect(primary).toHaveText('Разделить «Всё внутреннее пространство» на 2×1, полок: 3');
});
