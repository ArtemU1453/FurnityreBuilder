...
test('экспорт PDF и XLSX работает без сети (§6)', async ({ page, context }) => {
  /*
    Самый тяжёлый сценарий набора, и бюджет ему нужен соответствующий.

    Он ждёт догрузку полутора мегабайт в кэш, затем строит PDF с
    вложенным шрифтом и книгу XLSX — всё это уже без сети. Локально
    укладывается в две с половиной секунды, но бюджет по умолчанию у
    Playwright один на всех — тридцать секунд, столько же, сколько у
    теста с одним щелчком. На загруженном раннике CI, где сценарии идут
    параллельно, этого не хватило, и прогон упал не по существу.

    `test.slow()` утраивает бюджет. Это не смягчение проверки: ни одно
    утверждение не тронуто, и тест по-прежнему требует, чтобы оба файла
    действительно скачались без сети. Речь только о том, сколько времени
    отведено на заведомо долгую работу.
  */
  test.slow();
  await page.goto('./');
  await waitForServiceWorker(page);
