 ...
 * что лестница действительно переключает панели, что шаг «Производство»
...
test('каждый шаг открывает свою панель и только её (§27)', async ({ page }) => {
  ...
  const panels: readonly (readonly [string, string])[] = [
    ...
    // Шаг 10 с PROMPT 64 — панель конструктора, а не второй вход в
    // производство: у него своя панель, как у любого другого шага.
    ['Обзор', 'Обзор изделия'],
  ];
...
test('шаг «Производство» уводит на свой раздел и обратно возвращает туда же (§3)', async ({
  page,
}) => {
  await page.goto('./');
  await step(page, 'Материалы').click();

  // До PROMPT 64 экран производства открывали ДВА шага — 10 и 11, — и
  // проверялось это здесь на шаге «Проверка». Теперь пересекает границу
  // ровно один шаг, поэтому и проверяется он: шаг 10 остался в
  // конструкторе (`tests/e2e/final-steps.spec.ts`).
